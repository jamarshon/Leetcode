...
Therefore, they provide a functionality similar to vectors, but with efficient insertion and deletion of elements also at the beginning of
the sequence, and not only at its end. But, unlike vectors, deques are not guaranteed to store all its elements in contiguous storage
locations: accessing elements in a deque by offsetting a pointer to another element causes undefined behavior.
 
Both vectors and deques provide a very similar interface and can be used for similar purposes, but internally both work in quite different
ways: While vectors use a single array that needs to be occasionally reallocated for growth, the elements of a deque can be scattered in
different chunks of storage, with the container keeping the necessary information internally to provide direct access to any of its
elements in constant time and with a uniform sequential interface (through iterators). Therefore, deques are a little more complex
internally than vectors, but this allows them to grow more efficiently under certain circumstances, especially with very long sequences,
where reallocations become more expensive.
 
For operations that involve frequent insertion or removals of elements at positions other than the beginning or the end, deques perform
...
void DEQUE() {
    ...
    my_deque.assign (second.begin() + 1, second.end() - 1);    // copies input iterator first to last
    ...
    // all iterators related to this container are invalidated, but pointers and references remain valid,
    ...
    // Linear on the number of elements erased (destructions). Plus, depending on the particular library implemention,
    // up to an additional linear time on the number of elements between position and one of the ends of the deque.
    ...
    // If the erasure operation includes the last element in the sequence,
    // the end iterator and the iterators, pointers and references referring to the erased elements are invalidated.
    ...
    // Linear on the number of elements inserted (copy/move construction). Plus, depending on the particular library implemention,
    // up to an additional linear in the number of elements between position and one of the ends of the deque.
    // If the insertion happens at the beginning or the end of the sequence, all iterators related to this container are invalidated,
    // but pointers and references remain valid, referring to the same elements they were referring to before the call.
    // If the insertion happens anywhere else in the deque, all iterators, pointers and references related to this container are invalidated.
    ...
    auto fill_insert_it = my_deque.insert(my_deque.begin() + 3, 10, 4); // inserts 4 10 times at index 3
    assert(*fill_insert_it == 4);
 
    // range (3) template void insert (iterator position, InputIterator first, InputIterator last);
    auto range_insert_it = my_deque.insert(my_deque.begin() + 2, second.begin(), second.end()); // copies everything from second to my_deque starting at index 2
    assert(*range_insert_it == 100);
 
    // Assigns new contents to the container, replacing its current contents, and modifying its size accordingly.
    // Copies all the elements from x into the container. O(N)
    ...
    // pop and push adds element and removes element from front or back in constant time
    // pop reduces size of container by one while push increases it by one
 
    // The end iterator and any iterator, pointer and reference referring to the removed element are invalidated.
    // Iterators, pointers and references referring to other elements that have not been removed are guaranteed to keep
    // referring to the same elements they were referring to before the call.
    // void pop_back();
    my_deque.pop_back();
    ...
    // All iterators related to this container are invalidated. Pointers and references to elements in the container
    // remain valid, referring to the same elements they were referring to before the call.
    // void push_back (const value_type& val);
    ...
    // val, otherwise, they are value-initialized.
    // Notice that this function changes the actual content of the container by inserting or erasing elements from it.
    // Linear on the number of elements inserted/erased (constructions/destructions).
    // In case the container shrinks, all iterators, pointers and references to elements that have not been removed remain
    // valid after the resize and refer to the same elements they were referring to before the call.
    // If the container expands, all iterators are invalidated, but existing pointers and references remain valid, referring to
    // the same elements they were referring to before.
    ...
    // A deque container may have more memory allocated than needed to hold its current elements: this is because most
    // libraries implement deque as a dynamic array that can keep the allocated space of removed elements or allocate additional
    // capacity in advance to allow for faster insertion operations.
    // This function requests that the memory usage is adapted to the current size of the container, but the request is
    // non-binding, and the container implementation is free to optimize its memory usage otherwise.
    // Note that this function does not change the size of the container (for that, see resize instead).
    // O(N) No changes to iterators
    my_deque.assign(5,0);
    my_deque.resize(10);
...
// and iteration in both directions. List containers are implemented as doubly-linked lists; Doubly linked lists
// can store each of the elements they contain in different and unrelated storage locations. The ordering is kept
// internally by the association to each element of a link to the element preceding it and a link to the element following it.
...
// inserting, extracting and moving elements in any position within the container for which an iterator has already been
// obtained, and therefore also in algorithms that make intensive use of these, like sorting algorithms.
...
// distance between these. They also consume some extra memory to keep the linking information associated to each element
...
void LIST() {
    ...
    auto range_erase_it = my_list.erase(my_list.begin(), next(my_list.begin(), 1));
 
    // O(1) returns an iterator that points to the first of the newly inserted elements.
    ...
    auto range_insert_it = my_list.insert(my_list.begin(), second.begin(), second.end());
    ...
    // The function calls pred(*i) for each element (where i is an iterator to that element). Any
    // of the elements in the list for which this returns true, are removed from the container.
    // void remove_if (Predicate pred);
    for(auto e: my_list) cout << e << ' '; cout << endl;
    ...
    //Reverse the order of elements. Reverses the order of the elements in the list container. Linear in list size.
    // Iterator no changes.
    // void reverse();
    my_list.reverse();
    ...
    // or comp (in version (2)) to compare elements. This comparison shall produce a strict weak ordering of
    // the elements (i.e., a consistent transitive comparison, without considering its reflexiveness).
    // The resulting order of equivalent elements is stable: i.e., equivalent elements preserve the relative order
