...
public:
    ...
    // ie no intersection min(C,G) < max(A,E) just make right = left so width is zero 
    int computeArea(int A, int B, int C, int D, int E, int F, int G, int H) {
    ...
    int getNumber(const string& s, int& i, const int& N) {
        ...
        while(i < N) {
            if(isdigit(s[i])) {
                num = num*10 + (s[i] - '0');
                seen_digit = true;
            } else if(seen_digit) { // have seen a digit but current is not a digit meaning end of sequence
                break;
            }
            i++;
        }
