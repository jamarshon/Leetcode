...
class Solution {
 public:
  int maxProfit(vector<int>& prices) {
    ...
    /**
        dp[k][i] represents maximum profit of at most k transactions until index
       i dp[k][0] = 0 as no points dp[0][i] = 0 as no transactions dp[k][i] =
       max( dp[k][i-1], <- ignore element i price[i] + max{j = 0 to i}(-price[j]
       + dp[k-1][j]) <- take element i with element j and have k-1 transactions
       until index j
                )
    */
  ...
  int maxProfit3(vector<int>& prices) {
    int lowest_price1 = INT_MAX, lowest_price2 = INT_MAX, max_profit1 = 0,
        max_profit2 = 0;
  ...
  int maxProfit2(vector<int>& prices) {
    ...
    // forward[i] is max profit from [0, i] and backward[i] is max profit from
    // [i,N)
...
struct QueueItem {
  string vertex;
  vector<int> path;
  QueueItem(string _vertex, vector<int> _path = vector<int>{})
      : vertex(_vertex), path(_path) {}
};
 ...
 public:
  vector<vector<string>> findLadders(string beginWord, string endWord,
    ...
    for (auto path : paths) {
      ...
      transform(path.begin(), path.end(), temp.begin(),
                [wordList](const int index) { return wordList[index]; });
...
class Solution {
 ...
 public:
  int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
    ...
    for (auto word_it = distinct.begin(); word_it != distinct.end();
         word_it++) {
      for (auto match_word_it = next(word_it); match_word_it != distinct.end();
           match_word_it++) {
        string word = *word_it;
...
class Solution {
 public:
  void generate(vector<int>& dp, string str, vector<string>& temp, int index,
                vector<vector<string>>& res) {
    if (index == str.size() - 1) {
      const char kNullChar = '\0';
      vector<string> temp_copy(temp.size());
      transform(
          temp.begin(), temp.end(), temp_copy.begin(), [&kNullChar](string s) {
            s.erase(remove_if(
                        s.begin(), s.end(),
    ...
    for (int i = index + 1; i < str.size(); i++) {
      if (dp[i] > 0) {
        if (i - dp[i] <=
            index) {  // the palindrome extends to or past our current word so
                      // we can take a substring of it
          int end = i - index;
          string sub = str.substr(
              index, 2 * end);  // includes null at front but not back
          temp.push_back(sub);
