...
class Solution {
 public:
  double findMedianSortedArrays(vector<int>& C, vector<int>& D) {
    ...
    int imin = 0, imax = M,
        halfLen = (M + N + 1) /
                  2;  // this is middle index of the combined length of A and B
...
class Solution {
 public:
  vector<Denomination> VEC = {Denomination{1000, "M"}, Denomination{500, "D"},
                              Denomination{100, "C"},  Denomination{50, "L"},
                              Denomination{10, "X"},   Denomination{5, "V"},
                              Denomination{1, "I"}};

  void checkAndAddDenomination(int i, int offset, int len,
                               Denomination denomination, int& num,
                               string& result) {
    if (i + offset < len) {
