 ...
 public:
  bool canPlaceFlowers(vector<int>& flowerbed, int n) {
    ...
    for (int i = 1; i < v_len; i++) {
      ...
      // cout << v[i] << " " << v[i-1] << " " << num_zeros << " " << (num_zeros
      // - 1)/2 << endl;
      if (num_zeros > 0) {
...
class Solution {
 public:
  string tree2str(TreeNode* t) {
    ...
    if (left.empty() && right.empty()) return root;
    if (!left.empty() && right.empty()) return root + "(" + left + ")";
...
class Solution {
  TreeNode* mergeTreesHelper(TreeNode* t1, TreeNode* t2) {
    ...
    TreeNode* left =
        mergeTreesHelper(t1 ? t1->left : NULL, t2 ? t2->left : NULL);
    TreeNode* right =
        mergeTreesHelper(t1 ? t1->right : NULL, t2 ? t2->right : NULL);
    curr->left = left;
    curr->right = right;
    return curr;
  }
...
the deQueue(): Delete an element from the circular queue. Return true if the
...
non-empty arrays. The total number of the integers in all the m arrays will be
in the range of [2, 10000]. The integers in the m arrays will be in the range of
[-10000, 10000].
...
class Solution {
 public:
  int maxDistance(vector<vector<int>>& arrays) {
    ...
    sort(v.begin(), v.end(),
         [](Start e, Start b) { return e.first_value < b.first_value; });
    sort(v2.begin(), v2.end(),
         [](End e, End b) { return e.last_value > b.last_value; });
...
the range [-1000, 1000]. Multiplication of any three numbers in the input won't
...
of time. Note: Input logs will be sorted by timestamp, NOT log id. Your output
should be sorted by function id, which means the 0th element of your output
corresponds to the exclusive time of function 0. Two functions won't start or
end at the same time. Functions could be called recursively, and will always
end. 1 <= n <= 100
...
is 11. Hence return [3, 14.5, 11]. Note: The range of node's value is in the
 ...
 public:
  vector<double> averageOfLevels(TreeNode* root) {
    ...
    while (!q.empty()) {
      ...
      if (size1) res.push_back(sum / size1);
    }
