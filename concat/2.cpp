    ...
    for (int i = 0; i < len; i++) {
      ...
      if (s[i] == '(') {
        ...
        if (i == 0) {
          ...
          // '...))' check if it is '...((...))' then it is just the inner
          // length dp[i - 1] plus the two brackets completed on the outer plus
          // whatever was before the first (
...
class Solution {
  vector<pair<int, int>> getNeighborsDigits(const vector<vector<char>>& board,
                                            const int& i, const int& j) {
    function<bool(char)> is_digit = [](const char& c) -> bool {
      return isdigit(c);
    };
    return getNeighbors(board, i, j, is_digit);
  }

  vector<pair<int, int>> getNeighborsEmpty(const vector<vector<char>>& board,
                                           const int& i, const int& j) {
    function<bool(char)> is_empty = [](const char& c) -> bool {
      return c == '.';
    };
  ...
  bool canSolveSudoku(vector<vector<char>>& board,
    ...
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
        ...
        if (vn.num_avail > 0 &&
            (min_vn.num_avail == -1 || vn.num_avail < min_vn.num_avail)) {
          min_vn = vn;
          row = i;
          col = j;
        }
 ...
 public:
  void populateDof(const vector<vector<char>>& board,
                   vector<vector<ValidNums>>& dof) {
    for (int i = 0; i < 9; i++) {
...
int main() {
  Solution s;
  function<vector<char>(string)> to_v = [](string s) -> vector<char> {
    return vector<char>(s.begin(), s.end());
  };

  // vector<vector<char>>
  // board{to_v(".87654321"),to_v("2........"),to_v("3........"),to_v("4........"),to_v("5........"),to_v("6........"),to_v("7........"),to_v("8........"),to_v("9........")};
  vector<vector<char>> board{
      to_v("..9748..."), to_v("7........"), to_v(".2.1.9..."),
      to_v("..7...24."), to_v(".64.1.59."), to_v(".98...3.."),
      to_v("...8.3.2."), to_v("........6"), to_v("...2759..")};
  // vector<vector<char>>
  // board{to_v("..4...63."),to_v("........."),to_v("5......9."),to_v("...56...."),to_v("4.3.....1"),to_v("...7....."),to_v("...5....."),to_v("........."),to_v(".........")};
...
class Solution {
  void combinationSum(vector<int>& candidates, int sum,
                      vector<vector<int>>& res, vector<int>& curr, int index) {
