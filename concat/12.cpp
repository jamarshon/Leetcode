 ...
 public:
  ...
  bool search(string word) {
    ...
    while (!q.empty()) {
      ...
      if (word[index] ==
          '.') {  // increase index and add all 26 child to the queue
        for (int i = 0; i < 26; i++) {
...
class Solution {
 public:
  string Manacher(string s) {
    ...
    string suffix = "";
    max_str.erase(remove_if(max_str.begin(), max_str.end(),
                            [&kNullChar](char c) { return c == kNullChar; }),
                  max_str.end());
  ...
  string shortestPalindrome(string s) { return Manacher(s); }
...
class Solution2 {
 public:
  /* O(k) using quickselect
      using two ends of the array, slowly move towards the middle as low++,
     high-- when comparing to nums[0]
  */
  int findKthLargest(vector<int> nums, int k) {
    int low = 0;
    ...
    if (high > nums.size() - k) {
      return findKthLargest(vector<int>(nums.begin(), nums.begin() + high),
                            k - (nums.size() - high));
    } else {
      return findKthLargest(vector<int>(nums.begin() + high + 1, nums.end()),
                            k);
    }
  }
};
...
class Solution3 {
 public:
  /*
  O(k) using low_partition to be the right boundary of low meaning the first
  number > nums[0]
  */
  int findKthLargest(vector<int> nums, int k) {
    int low_partition = 0;
    ...
    if (pivot_ind > nums.size() - k) {
      return findKthLargest(vector<int>(nums.begin(), nums.begin() + pivot_ind),
                            k - (nums.size() - pivot_ind));
    } else {
      return findKthLargest(
          vector<int>(nums.begin() + pivot_ind + 1, nums.end()), k);
    }
  }
};
...
class Solution {
 public:
  void recurse(int current, int sum, int K, int N, vector<int>& temp,
...
class Solution2 {
 public:
  vector<vector<int>> combinationSum3(int K, int N) {
    ...
    vector<vector<vector<vector<int>>>> dp(K + 1,
                                           vector<vector<vector<int>>>(N + 1));
    // dp[k][i] represents all the vectors that add up to i of length k that is
    // in ascending order using numbers [1, 9] dp[0][i] and dp[k][0] is empty
    // dp[1][i] is just [i]
...
10000]. The input list is already sorted in ascending order by the left x
...
struct SortCompare {
  bool operator()(const Item& lhs, const Item& rhs) {
    ...
    if (lhs.is_start) {
      // both start put higher building first cause it overshadows the smaller
      // ones
      return lhs.y > rhs.y;
    } else {
      // both end put smaller building first as we want to remove all the
      // smaller ones first
      return lhs.y < rhs.y;
    }
  }
...
class Solution {
 public:
  vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
    ...
    // for(auto item: items) cout << item.x << ' ' << item.y << ' ' <<
    // item.is_start << endl;
    for (auto item : items) {
