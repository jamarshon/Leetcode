...
class Solution {
 public:
  /*
      let dp[n] denote a number with n digits (no leading zeros) that is
     completely unique dp[0] = 1           {0} dp[1] = 9           {1, 2, 3, 4,
     5, 6, 8, 9} dp[2] = 9*dp[1]     for each number in dp[1], there are 9
     numbers to choose from that are not in dp[1]. e.g. for dp[1] element 1,
     there is {10, 12, 13, 14, 15, 16, 17, 18, 19} dp[3] = 8*dp[2]     for each
     number in dp[2], there are 8 numbers to choose from that are not in dp[2]
                          e.g. for dp[2] element 12, there is {120, 123, 124,
     125, 126, 127, 128, 129}
      ...
...
class Solution {
 public:
  /*
      suppose the number we have is right after <, if we meet a smaller element
     then just add it to stack else if the element is larger than we pop back
     and add this larger element as it will give a wider range of numbers that
     can follow it the same logic applies to >, where if a smaller element is
     seen just pop back and add it to stack
  */
